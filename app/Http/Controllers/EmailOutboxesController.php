...
class EmailOutboxesController extends Controller implements Paginatable
{
    use AcceptsPagination;

    protected $CoreEmailOutbox;
    public function __construct(CoreEmailOutbox $CoreEmailOutbox){
        $this->db_table = $CoreEmailOutbox;
    }
    ...
    public function validator(Request $request)
    {
        ...
        $input = [
            ...
            'subject'=> $this->sanitizer($request->input('subject')),
            'content'=> $this->sanitizer($request->input('content')),
            'remarks'=> $this->sanitizer($request->input('remarks')),
        ];
    ...
    public function controller_variables(){
        ...
        $controller_res['site_title'] = 'Email Outbox'; 
        $controller_res['folder_name'] = 'email_outbox';
    ...
    public function table_columns(){
        ...
        $table_col_display = [];
        $table_col_display[] = ['table_name' => 'email','headerName'=> 'EMAIL','is_display'=>true,'is_widen'=>false, 'column_width'=>'W_WIDE'];
        $table_col_display[] = ['table_name' => 'subject','headerName'=> 'SUBJECT','is_display'=>true,'is_widen'=>false, 'column_width'=>'W_WIDE'];
    ...
    public function store(Request $request)
    {
        ...
        if($accesslevel->add){
            ...
            $email = $this->pushEmail($email, $subject, $content);
            // $email = $this->pushEmail('strife server', 'subject test', 'message test test');
            if($email){
                $core_audit_trail = $this->audit_trail('','TEST SEND EMAIL',$validator,'');
            }
    ...
    // public function update(Request $request, CoreEmailOutbox $CoreEmailOutbox)
    // {
    //     $accesslevel = json_decode($this->acclvl());
    ...
    //         $id = $this->sanitizer($request->input('id'));
    //         $validator = $this->validator($request);
    //         $data = $this->db_table->findOrFail($id)->update($validator);
    //         $this->audit('','','',$id);
    //         return redirect()->route($page_variables['update_page'])
    //             ->with('success',$page_variables['title'].' Updated Successfully');
    //     }else{
