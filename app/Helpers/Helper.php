...
class Helper
{
    ...
    public static function email_send(array $data): array
    {
        $returns = [];
        $service = app(MailService::class);
        $service_execute = $service->payloading($data);
        return $service_execute;
    }
    ...
    public static function applClasses()
    {
        ...
        if(Schema::hasTable('core_navigations')){
            $navs = DB::table('core_navigations')
                 ->where('nav_type', '=','sub')
                 ->orWhere('nav_type', '=','single')
                 ->get();

            // $navlist = array();
            //      foreach ($navs as $key => $value) {
            //         $navlist[] = array("name"=>$value->nav_name,"url"=>($value->nav_mode == '/') ? '/ ' : $value->nav_mode,"icon"=>$value->nav_icon);
            //      }
            //      $search_nav = array('listItems'=>'');
            //      $search_nav['listItems'] = $navlist;
            // $file = fopen('../public/data/laravel-search-list.json', 'r');
            // $dbnav_items = count($search_nav['listItems']);
            // $json_items = count(json_decode(fgets($file))->listItems);

            // fclose($file);

            // if($dbnav_items != $json_items){
            //     $fp = fopen('../public/data/laravel-search-list.json', 'w');
            //     fwrite($fp, json_encode($search_nav,JSON_UNESCAPED_SLASHES));
            //     fclose($fp);
            // }

            $titles = DB::table('core_settings')
                 ->where('setting_name', '=','site_title')
                 ->Orwhere('setting_name', '=','footer_title')
                 ->Orwhere('setting_name', '=','page_logo')
                 ->orderBy('id')
                 ->get();
        ...
        $layoutClasses = [
            ...
            'site_title' => $titles[0]->setting_value,
            'footer_title' => $titles[1]->setting_value,
            'page_logo' => $titles[2]->setting_value
