...
$(document).ready(function() {
  ...
  columnDefs = {
    ...
    cellRenderer: function(pa) {
      var url = '{{ route("user_levels.edit", ":id") }}';
      url = url.replace(':id', pa.value);

      var url2 = '{{ route("user_levels.destroy", ":id") }}';
      ...
      if(controls.edit){
        var eButton = eDiv.querySelectorAll('.update_btn')[0];
      }
      if(controls.delete){
        var eButton1 = eDiv.querySelectorAll('.delete_btn')[0];
      }
      if(controls.edit){
      ...
      if(controls.delete){
        eButton1.addEventListener('click', function() {
            var data_id = $(this).data("id");
            var url = '{{ route("user_levels.destroy", ":id") }}';
            url = url.replace(':id', data_id);
    ...
    // tree view renderer
  var treeModuleRenderer = {
    headerName: 'Allowed Modules',
    field: 'allow_module_html',
    // cellStyle: { height: '100px' },
    autoHeight: true,
  ...
  grid_data.column.push(treeModuleRenderer);
...
//console.log(grid_data.column)
  var gridOptions = {
      ...
      onGridReady: function () {
          ...
          gridOptions.columnApi.moveColumnByIndex(2, 0);
      }
  };
  ...
  $(".ag-grid-export-btn").on("click", function(params) {
    var dateNow = function() {
      var x = new Date();
      return (x.getMonth() + 1 < 9 ? '0' + x.getMonth()+1 : x.getMonth()+1)
      + "" + x.getDate()
      + "" + x.getYear()
    }

    let options = {
      fileName: window._appVars._config.user_level + dateNow() + '.csv',
      columnKeys: [ 'accesslevel', 'allowed_modules_fmt' ]
    }

   gridOptions.api.exportDataAsCsv(options);
  });
