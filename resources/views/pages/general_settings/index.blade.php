    ...
    <section id="">
                                        ...
                                        @if ($data_filter['filter_type'] == 'dropdown_filter')
                                            <div class="col-12 col-sm-6 col-lg-3">
                                                ...
                                                <fieldset class="form-group">
                                                    <select name="{{ $data_filter['filter_code'] }}"
                                                        class="filter_data select2 form-control {{ $data_filter['filter_code'] }}"
                                                        id="{{ $data_filter['filter_code'] }}">
                                                        <option value="">All</option>
                                                        @foreach ($data_filter['dropdown_data'] as $dropdown_data)
                                                            <option value="{{ $dropdown_data['value'] }}">
                                                                {{ $dropdown_data['status_name'] }}</option>
                                                        @endforeach
                                                    </select>
                                                </fieldset>
                                            </div>
                                            ...
                                            <div class="col-12 col-sm-6 col-lg-3">
                                                ...
                                                <fieldset class="form-group">
                                                    <input type="text" name="from" id="from"
                                                        class="filter_data form-control format-picker"
                                            ...
                                            <div class="col-12 col-sm-6 col-lg-3">
                                                ...
                                                <fieldset class="form-group">
                                                    <input type="text" name="to" id="to"
                                                        class="filter_data form-control format-picker"
                                                        id="cal-start-date to" readonly="" placeholder="MM-DD-YYYY">
                                                </fieldset>
                                            </div>
                                        @endif
                                    @endforeach
        ...
        <div class="card">
            <form class="form form-horizontal" enctype="multipart/form-data" method="POST"
                action="{{ route('general_settings.store') }}">
                @csrf
                <input type="text"readonly name="model[][value]" value="1111">
                <input type="text"readonly name="model[][value]" value="22222">
                <fieldset class="form-group">

                    <input id="import_upload" type="file" name="import_file">
                </fieldset>
...
@endforeach
                                ...
                                @if (session()->get('error_comment'))
                                    <div id="error_active" class="alert alert-danger">
                                        ...
                                        @if (session()->get('error_link'))
                                            <a id="download_errors" href="{{ session()->get('error_link') }}  ">Click here if
                                                the file didn't download Automatically</a>
                                        @endif
                                    </div>
                                @endif
                            @endcan
                                                ...
                                                <div class="dropdown-menu">
                                                    @can('add')
                                                        <a class="dropdown-item "
                                                            href="{{ route('general_settings.create') }}"><i
                                                                class="feather icon-clipboard"></i> Create</a>
                                                    @endcan
                                                    @can('export')
                                                        <a class="dropdown-item  ag-grid-export-btn" href="#"><i
                                                                class="feather icon-download"></i> Export as CSV</a>
                                                    @endcan
                                                </div>
                                            </div>
                                        </div>
    ...
    <div class="modal fade" id="custom_modal" tabindex="-1" role="dialog" aria-labelledby="custom_modal_title"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-centered modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="custom_modal_title">Title</h5>

                </div>
                <div id="custom_modal_content"class="modal-body" style="color: black;">

                </div>
                <div class="modal-footer">
                    <form id="custom_form" enctype="multipart/form-data" class="form form-horizontal" method="POST"
                        action="">
                        @csrf
                        <!-- @method('PUT') -->
                        <input type="text" name="user_id" id="user_id" readonly hidden value="1">
