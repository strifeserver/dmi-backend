...
function makeControls() {
  return {
      headerName:  '\xa0\xa0\xa0\xa0\xa0\xa0' +'CONTROLS' + '\xa0\xa0\xa0\xa0\xa0\xa0\xa0',
      field: 'id',
      width: 500,
      cellRendererParams: {
          routes: {
      ...
      cellRenderer: function (params) {
        ...
        //  if(params.data.full_url){
          // for adding buttons
            const custom_btn_modal_1 = CORE_Framework.createcustom_btn1({ id: params.value,button_title: 'Reset password' ,btn_type: 'btn-info' ,icon: 'fa fa-key' })
            custom_btn_modal_1.addEventListener('click', () => {
            ...
            root.appendChild(custom_btn_modal_1)

            const anchor = document.createElement('a');
            anchor.href = params.data.full_url;
            anchor.innerText = anchor;
            params.data.full_url = anchor;


            const custom_btn_modal_2 = CORE_Framework.createcustom_btn1({ id: params.value,button_title: 'Reset password' ,btn_type: 'btn-info' ,icon: 'fa fa-key' })
            custom_btn_modal_2.addEventListener('click', () => {
                document.getElementById("user_id").value = params.value
                document.getElementById("custom_modal_title").innerHTML = 'Reset Password?';
                document.getElementById("custom_modal_content").innerHTML = 'Confirm Reset Password?';
            });
            root.appendChild(custom_btn_modal_2)

            const anchor_b = document.createElement('a');
            anchor_b.href = params.data.full_url;
            anchor_b.innerText = anchor_b;
            params.data.full_url = anchor_b;
